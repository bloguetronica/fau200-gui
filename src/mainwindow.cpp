...
void MainWindow::on_actionSerial_triggered()
{
    ...
    if (serial.exec() == QDialog::Accepted)
    {
        ...
        if (serialstr_.isEmpty())
            this->setWindowTitle("FAU200 Power Supply");
        else
            this->setWindowTitle("FAU200 Power Supply (S/N: " + serialstr_ + ")");
    }
}
...
void MainWindow::on_doubleSpinBoxVolt_valueChanged(const QString &arg)
{
    if (ui->actionAutoApply->isChecked())
        execute(("fau200-volt " + arg + " " + serialstr_).simplified());
}

void MainWindow::on_pushButtonApply_clicked()
{
    QString volt = QString::number(ui->doubleSpinBoxVolt->value());
    execute(("fau200-volt " + volt + " " + serialstr_).simplified());
}

void MainWindow::on_pushButtonClear_clicked()
{
    execute(("fau200-clear " + serialstr_).simplified());
}

void MainWindow::on_pushButtonReset_clicked()
{
    if (ui->actionResetClear->isChecked())
        execute(("fau200-reset " + serialstr_ + " && sleep 2 && fau200-clear " + serialstr_).simplified());
    else
        execute(("fau200-reset " + serialstr_).simplified());
}
